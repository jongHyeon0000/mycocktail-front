...
const useReadReceivedLikes = () => {
  const [response, setResponse] = useState<ApiResponse<ReceivedLike[] | null> | undefined>(undefined);
  const [loading, setLoading] = useState<boolean>(false);
  const [loadingMore, setLoadingMore] = useState<boolean>(false);
  const [hasMore, setHasMore] = useState<boolean>(true);

  const fetchReadReceivedLikes = async (params?: FetchProps, appendData = false) => {
    if (appendData) {
      setLoadingMore(true);
    } else {
    ...
    try {
      ...
      const res = await api.get<ApiResponse<ReceivedLike[]>>(`/api/user/${userId}/received-likes`, { params: queryParams });
  ...
  return {
    receivedLikeList: response,
    receivedLikeListLoading: loading,
    receivedLikeListLoadingMore: loadingMore,
    receivedLikeListHasMore: hasMore,
    fetchReadReceivedLikes
  };
}

export default useReadReceivedLikes;
