...
const UserCarbonatedInsertModal: React.FC<UserCarbonatedInsertModalProps> = ({ open, onClose, onSelect }) => {
  const {
    carbonatedList,
    carbonatedListLoading,
    carbonatedListHasMore,
    fetchReadCarbonatedList,
  } = useReadCarbonatedList();
  ...
  useEffect(() => {
    if (open) {
      setPage(1);
      setSearch("");
      fetchReadCarbonatedList({ page: 1, limit: PAGE_SIZE });
    }
  }, [open]);
  ...
  const handleSearch = (value: string) => {
    setSearch(value);
    setPage(1);
    fetchReadCarbonatedList({ page: 1, limit: PAGE_SIZE, search: value });
  };

  const handlePageChange = (newPage: number) => {
    setPage(newPage);
    fetchReadCarbonatedList({ page: newPage, limit: PAGE_SIZE, search });
  };
    ...
    <AnimatePresence>
      {open && (
        <StyledModal open={open} onClose={onClose} closeAfterTransition>
          ...
          >
            <ModalHeader>
              <Typography variant="h6" fontWeight={700}>탄산/소다 추가</Typography>
            ...
            <PaginationArea>
              ...
              <PageArrowBtn
                onClick={() => handlePageChange(page + 1)}
                disabled={!carbonatedListHasMore}
              >
                ›
              </PageArrowBtn>
            </PaginationArea>
