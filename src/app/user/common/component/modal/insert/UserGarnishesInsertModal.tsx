...
const UserGarnishesInsertModal: React.FC<UserGarnishesInsertModalProps> = ({ open, onClose, onSelect }) => {
  const {
    garnishesList,
    garnishesListLoading,
    garnishesListHasMore,
    fetchReadGarnishesList,
  } = useReadGarnishesList();
  ...
  useEffect(() => {
    if (open) {
      setPage(1);
      setSearch("");
      fetchReadGarnishesList({ page: 1, limit: PAGE_SIZE });
    }
  }, [open]);
  ...
  const handleSearch = (value: string) => {
    setSearch(value);
    setPage(1);
    fetchReadGarnishesList({ page: 1, limit: PAGE_SIZE, search: value });
  };

  const handlePageChange = (newPage: number) => {
    setPage(newPage);
    fetchReadGarnishesList({ page: newPage, limit: PAGE_SIZE, search });
  };
