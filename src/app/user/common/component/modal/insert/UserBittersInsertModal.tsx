...
const UserBittersInsertModal: React.FC<UserBittersInsertModalProps> = ({ open, onClose, onSelect }) => {
  ...
  useEffect(() => {
    if (open) {
      setPage(1);
      setSearch("");
      fetchReadBittersList({ page: 1, limit: PAGE_SIZE });
    }
  }, [open]);
  ...
  const handleSearch = (value: string) => {
    setSearch(value);
    setPage(1);
    fetchReadBittersList({ page: 1, limit: PAGE_SIZE, search: value });
  };

  const handlePageChange = (newPage: number) => {
    setPage(newPage);
    fetchReadBittersList({ page: newPage, limit: PAGE_SIZE, search });
  };
    ...
    <AnimatePresence>
      {open && (
        <StyledModal open={open} onClose={onClose} closeAfterTransition>
          ...
          >
            <ModalHeader>
              <Typography variant="h6" fontWeight={700}>비터스 추가</Typography>
