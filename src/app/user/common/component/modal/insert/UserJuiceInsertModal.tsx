...
const UserJuiceInsertModal: React.FC<UserJuiceInsertModalProps> = ({ open, onClose, onSelect }) => {
  ...
  useEffect(() => {
    if (open) {
      setPage(1);
      setSearch("");
      fetchReadJuiceList({ page: 1, limit: PAGE_SIZE });
    }
  }, [open]);
  ...
  const handleSearch = (value: string) => {
    setSearch(value);
    setPage(1);
    fetchReadJuiceList({ page: 1, limit: PAGE_SIZE, search: value });
  };

  const handlePageChange = (newPage: number) => {
    setPage(newPage);
    fetchReadJuiceList({ page: newPage, limit: PAGE_SIZE, search });
  };
    ...
    <AnimatePresence>
      {open && (
        <StyledModal open={open} onClose={onClose} closeAfterTransition>
          ...
          >
            <ModalHeader>
              <Typography variant="h6" fontWeight={700}>주스 추가</Typography>
