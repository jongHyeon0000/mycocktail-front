...
const UserToolInsertModal: React.FC<UserToolInsertModalProps> = ({ open, onClose, onSelect }) => {
  ...
  useEffect(() => {
    if (open) {
      setPage(1);
      setSearch("");
      fetchReadToolList({ page: 1, limit: PAGE_SIZE });
    }
  }, [open]);
  ...
  const handleSearch = (value: string) => {
    setSearch(value);
    setPage(1);
    fetchReadToolList({ page: 1, limit: PAGE_SIZE, search: value });
  };

  const handlePageChange = (newPage: number) => {
    setPage(newPage);
    fetchReadToolList({ page: newPage, limit: PAGE_SIZE, search });
  };
    ...
    <AnimatePresence>
      {open && (
        <StyledModal open={open} onClose={onClose} closeAfterTransition>
          ...
          >
            <ModalHeader>
              <Typography variant="h6" fontWeight={700}>도구/기물 추가</Typography>
