...
const UserTechniqueInsertModal: React.FC<UserTechniqueInsertModalProps> = ({ open, onClose, onSelect }) => {
  const {
    techniqueList,
    techniqueListLoading,
    techniqueListHasMore,
    fetchReadTechniqueList,
  } = useReadTechniqueList();
  ...
  useEffect(() => {
    if (open) {
      setPage(1);
      setSearch("");
      fetchReadTechniqueList({ page: 1, limit: PAGE_SIZE });
    }
  }, [open]);
  ...
  const handleSearch = (value: string) => {
    setSearch(value);
    setPage(1);
    fetchReadTechniqueList({ page: 1, limit: PAGE_SIZE, search: value });
  };

  const handlePageChange = (newPage: number) => {
    setPage(newPage);
    fetchReadTechniqueList({ page: newPage, limit: PAGE_SIZE, search });
  };
    ...
    <AnimatePresence>
      {open && (
        <StyledModal open={open} onClose={onClose} closeAfterTransition>
          ...
          >
            <ModalHeader>
              <Typography variant="h6" fontWeight={700}>제조 기법 추가</Typography>
