...
const UserGlasswareInsertModal: React.FC<UserGlasswareInsertModalProps> = ({ open, onClose, onSelect }) => {
  const {
    glasswareList,
    glasswareListLoading,
    glasswareListHasMore,
    fetchReadGlasswareList,
  } = useReadGlasswareList();
  ...
  useEffect(() => {
    if (open) {
      setPage(1);
      setSearch("");
      fetchReadGlasswareList({ page: 1, limit: PAGE_SIZE });
    }
  }, [open]);
  ...
  const handleSearch = (value: string) => {
    setSearch(value);
    setPage(1);
    fetchReadGlasswareList({ page: 1, limit: PAGE_SIZE, search: value });
  };

  const handlePageChange = (newPage: number) => {
    setPage(newPage);
    fetchReadGlasswareList({ page: newPage, limit: PAGE_SIZE, search });
  };
    ...
    <AnimatePresence>
      {open && (
        <StyledModal open={open} onClose={onClose} closeAfterTransition>
          ...
          >
            <ModalHeader>
              <Typography variant="h6" fontWeight={700}>글라스웨어 추가</Typography>
