...
const UserOtherIngredientsInsertModal: React.FC<UserOtherIngredientsInsertModalProps> = ({ open, onClose, onSelect }) => {
  const {
    ...
    otherIngredientsListLoading,
    otherIngredientsListHasMore,
    fetchReadOtherIngredientsList,
  } = useReadOtherIngredientsList();
  ...
  useEffect(() => {
    if (open) {
      setPage(1);
      setSearch("");
      fetchReadOtherIngredientsList({ page: 1, limit: PAGE_SIZE });
    }
  }, [open]);
  ...
  const handleSearch = (value: string) => {
    setSearch(value);
    setPage(1);
    fetchReadOtherIngredientsList({ page: 1, limit: PAGE_SIZE, search: value });
  };

  const handlePageChange = (newPage: number) => {
    setPage(newPage);
    fetchReadOtherIngredientsList({ page: newPage, limit: PAGE_SIZE, search });
  };
    ...
    <AnimatePresence>
      {open && (
        <StyledModal open={open} onClose={onClose} closeAfterTransition>
          ...
          >
            <ModalHeader>
              <Typography variant="h6" fontWeight={700}>기타 재료 추가</Typography>
            ...
            <PaginationArea>
              ...
              <PageArrowBtn
                onClick={() => handlePageChange(page + 1)}
                disabled={!otherIngredientsListHasMore}
              >
                ›
              </PageArrowBtn>
            </PaginationArea>
