...
interface SyrupListComponentProps {
  data: SyrupDetail;
  index: number;
  onClickEvent: () => void;
}

const SyrupListComponent: React.FC<SyrupListComponentProps> = ({ data, index, onClickEvent }) => {
  return (
      <Card
          key={`${data.syrupId}-${index}`}
          ...
          <ContentSection>
            <Box>
              <TechniqueTitle variant="h6">
                {data.syrupNameKr} ({data.syrupName})
              </TechniqueTitle>
...
export default SyrupListComponent;
