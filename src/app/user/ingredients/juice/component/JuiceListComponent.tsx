...
interface JuiceListComponent {
  data: JuiceDetail;
  index: number;
  onClickEvent: () => void;
}

const JuiceListComponent: React.FC<JuiceListComponent> = ({ data, index, onClickEvent }) => {
  return (
      <Card
          key={`${data.juiceId}-${index}`}
          ...
          <ContentSection>
            <Box>
              <TechniqueTitle variant="h6">
                {data.juiceNameKr} ({data.juiceName})
              </TechniqueTitle>
...
export default JuiceListComponent;
