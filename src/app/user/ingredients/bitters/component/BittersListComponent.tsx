...
interface BittersListComponentProps {
  data: BittersDetail;
  index: number;
  onClickEvent: () => void;
}

const BittersListComponent: React.FC<BittersListComponentProps> = ({ data, index, onClickEvent }) => {
  return (
      <Card
          key={`${data.bittersId}-${index}`}
          ...
          <ContentSection>
            <Box>
              <ItemTitle variant="h6">
                {data.bittersNameKr} ({data.bittersName})
              </ItemTitle>
...
export default BittersListComponent;
