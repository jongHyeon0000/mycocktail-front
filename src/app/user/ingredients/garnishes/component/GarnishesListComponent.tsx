...
interface GarnishesListComponent {
  data: GarnishesDetail;
  index: number;
  onClickEvent: () => void;
}

const GarnishListComponent: React.FC<GarnishesListComponent> = ({ data, index, onClickEvent }) => {
  return (
      <Card
          key={`${data.garnishId}-${index}`}
          ...
          <ContentSection>
            <Box>
              <TechniqueTitle variant="h6">
                {data.garnishNameKr} ({data.garnishName})
              </TechniqueTitle>
...
export default GarnishListComponent;
